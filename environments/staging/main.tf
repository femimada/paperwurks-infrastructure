...
# -----------------------------------------------------------------------------
# IAM SCOPING: Grant Deploy Role access to Staging ECS Resources
# -----------------------------------------------------------------------------
...
data "aws_iam_policy_document" "staging_ecs_deploy" {
  statement {
    effect = "Allow"
    actions = [
...
resource "aws_iam_policy" "staging_ecs_deploy_policy" {
  ...
  policy      = data.aws_iam_policy_document.staging_ecs_deploy.json
}

resource "aws_iam_role_policy_attachment" "deploy_role_staging_ecs" {
  role       = data.aws_iam_role.deploy_role.name
  policy_arn = aws_iam_policy.staging_ecs_deploy_policy.arn
}